/** Map DB aggregator row + nested trades to API list/detail shape. */
export function serializeAggregatorRow(a: Record<string, unknown>, trades?: unknown[]) {
  const tradeList = (Array.isArray(trades) ? trades : (a.trades as unknown[]) || []) as Array<{
    stage?: string;
    contract_value_usd?: number;
    settled_at?: string | null;
  }>;
  const completedTrades = tradeList.filter((t) => t.stage === 'SETTLED');
  const score = (a.creditworthiness_score as number | null) ?? 70;
  const credit_rating =
    score >= 90 ? 'AAA' : score >= 80 ? 'AA' : score >= 70 ? 'A' : score >= 60 ? 'BBB' : 'BB';

  return {
    id: (a.id as string) || 'unknown',
    name: (a.name as string) || 'Unnamed Aggregator',
    country: (a.country as string) || 'Unknown',
    registration_no: (a.registration_no as string | null) ?? null,
    notes: (a.notes as string | null) ?? null,
    sanctions_clear: Boolean(a.sanctions_clear),
    sanctions_checked_at: (a.sanctions_checked_at as string | null) ?? null,
    creditworthiness_score: score,
    credit_rating,
    trades_completed: completedTrades.length,
    trades_on_time: completedTrades.length,
    disputes: (a.disputes as number) || 0,
